...
async function submitQuestion(event) {
  ...
  try {
    ...
    if (requestedMode === 'agent' || result.trace.length > 1) {
      ...
      for (const step of result.trace) list.append(node('li', (stepNames[step.tool] || step.tool) + (step.query ? ': ' + step.query : '') + (step.method === 'explicit_reference' ? ' · açık gönderme doğrudan çözüldü' : '') + (step.reason ? ' · ' + (reasonNames[step.reason] || 'Bağlam doğrulaması başarısız') : '') + (step.found !== undefined ? ' · ' + step.found + (step.required !== undefined ? '/' + step.required + ' kanıt' : ' sonuç') : '')));
