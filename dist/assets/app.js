...
const state = { user: null, subjects: [], subject: null, documents: [], view: 'study', busy: false, deleteId: null, timer: null };
...
function showLogin() { state.user = null; $('login-view').hidden = false; $('app-view').hidden = true; $('password').value = ''; }
function setView(view) {
  ...
  $('page-eyebrow').textContent = { study: 'ÇALIŞMA MASASI', documents: 'DOKÜMAN YÖNETİMİ', system: 'YEREL ALTYAPI' }[view];
...
function clearChat() {
  ...
  welcome.append(node('span', '01 / KEŞFET', 'section-number'), node('h3', 'Hangi konuyu netleştirelim?'), node('p', 'Sorunda konuyu açıkça belirt. Her soru bağımsız işlenir; yalnızca seçili dersin hazır dokümanları aranır.'));
...
function renderSources(sources) {
  ...
  for (const [index, source] of sources.entries()) {
    ...
    card.append(node('span', source.source_id || 'K' + (index + 1), 'source-label'), node('h3', source.filename), node('small', source.location));
...
async function refreshDocuments() {
  ...
  if (selected !== state.subject) return;
  ...
  $('send').disabled = state.busy || !docs.some(d => d.status === 'ready' && !d.needs_reindex);
  if ($('welcome-text')) $('welcome-text').textContent = docs.some(d => d.status === 'ready' && !d.needs_reindex) ? 'Notların hazır. Bir kavram sor, iki konuyu karşılaştır veya belirli bir dönemi açıklat.' : 'Önce Dokümanlar bölümünden notlarını ekle. İşleme tamamlandığında soru sorabilirsin.';
}
...
async function submitQuestion(event) {
  ...
  if (state.busy || !state.subject || question.length < 3) return;
  const requestUserId = state.user?.id;
  state.busy = true; $('send').disabled = true; $('subject-select').disabled = true; $('clear-chat').disabled = true; $('add-subject').disabled = true; $('question').value = '';
  $('messages').querySelector('.welcome')?.remove();
  const item = node('article', undefined, 'message'); item.append(node('div', question, 'message-user'));
  const pending = node('p', $('mode').value === 'agent' ? 'Ajan seçili dersin kaynaklarını araştırıyor…' : 'Notlar aranıyor, yerel model yanıtı hazırlıyor…', 'pending'); item.append(pending); $('messages').append(item); pending.scrollIntoView({ block: 'nearest' });
  try {
    const result = await api('/api/questions', { method: 'POST', body: { subject_id: state.subject, question, mode: $('mode').value } });
    ...
    item.append(actions);
    if (result.trace.length > 1) { const details = node('details', undefined, 'trace'); details.append(node('summary', 'Araç işlemlerini göster')); const list = node('ol'); for (const step of result.trace) list.append(node('li', step.tool + ': ' + (step.query || 'İzin verilmeyen işlem') + ' · ' + step.found + ' sonuç')); details.append(list); item.append(details); }
    renderSources(result.sources);
  } catch (e) { pending.remove(); item.append(node('p', e.message, 'error')); $('question').value = question; }
  finally { state.busy = false; $('subject-select').disabled = false; $('clear-chat').disabled = false; $('add-subject').disabled = false; if (state.user) await refreshDocuments().catch(() => {}); }
}
...
$('subject-select').addEventListener('change', () => { state.subject = $('subject-select').value; clearChat(); $('question').value = ''; setView(state.view); refreshDocuments().catch(e => toast(e.message, true)); });
...
$('subject-form').addEventListener('submit', async event => { event.preventDefault(); try { const subject = await api('/api/subjects', { method: 'POST', body: { name: $('subject-name').value.trim() } }); $('subject-dialog').close(); $('subject-name').value = ''; clearChat(); await loadSubjects(subject.id); toast('Yeni ders oluşturuldu.'); } catch (e) { toast(e.message, true); } });
...
if (document.modelContext?.registerTool) {
  ...
  try {
    Promise.resolve(document.modelContext.registerTool({ name: 'select_study_subject', title: 'Çalışılacak dersi seç', description: 'Mevcut yetkili derslerden birini arayüzde seçer; veri oluşturmaz veya silmez. Açık cevap görünümünü temizler.', inputSchema: { type: 'object', properties: { subject_id: { type: 'string' } }, required: ['subject_id'], additionalProperties: false }, annotations: { readOnlyHint: false, untrustedContentHint: true }, async execute(input) { if (state.busy) throw new Error('Soru işlenirken ders değiştirilemez.'); if (!input || typeof input.subject_id !== 'string' || !state.subjects.some(s => s.id === input.subject_id)) throw new Error('Yetkili bir ders seç.'); state.subject = input.subject_id; $('subject-select').value = state.subject; clearChat(); setView('study'); await refreshDocuments(); return { selected_subject: currentSubject().name, documents: state.documents.length }; } }, { signal: lifecycle.signal })).catch(() => {});
