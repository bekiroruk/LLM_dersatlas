...
const context = vm.createContext({
  ...
  async fetch(url, options) {
    ...
    else if (url === '/api/system') data = { model: { reachable: true, chat_ready: true, embed_ready: true }, qdrant_ready: true, max_upload_mb: 20, worker_enabled: true, rag_revision: '2026-09-20-source-contract-v13', database: 'SQLite', version: '0.1.0', chat_model: 'qwen3:4b', embedding_model: 'bge-m3', vector_mode: 'Qdrant gömülü' };
...
async function main() {
  ...
  check('Çalışan RAG sürümü ana ekranda görünür', () => {
    assert.ok(get('model-badge').textContent.includes('RAG v13'));
    assert.ok(get('model-badge').title.includes('source-contract-v13'));
  });
